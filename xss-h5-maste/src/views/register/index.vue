    ...
    <div class="container">
      ...
      <div class="from-group">
        ...
        <div class="padding-15 mt20">
             <x-button @click.native="register"  type="primary" class="color_base">注册</x-button>
        </div>
     </div>
      <div class="div-tip mt20 padding-15">
       <check-icon :value.sync="agree"  class="f14 mt10"> 阅读代表同意 </check-icon>
       <router-link :to="{path:'/register/useragreement/index'}" class="agreement color_base" style="vertical-align: -2px;">《手机绑定协议》</router-link>
     </div>
    </div>
  </div>
</template>
...
//        console.log("pwd:" + pwd)
        if(this.mobile==""){
            ...
            // 调用接口
             get(url.sendSmsCode, params).then(response => {
      ...
      register(){
        if(this.mobile==""){
          ...
          this.$vux.toast.text('请同意手机绑定协议', 'middle')
        }else{
          const params = {
            mobile:this.mobile,
          ...
          // 调用接口
           post(url.register, params).then(response => {
