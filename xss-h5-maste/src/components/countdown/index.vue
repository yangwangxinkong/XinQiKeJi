<template>
  <div>
    <p v-if="msTime.show" class="f12">
      <!-- 开始提示 -->
      <!-- <span class="grey_3 " v-if="tipShow">{{tipTextStart}}：</span> -->
      <!-- 结束提示 -->
      <span class="grey_3" v-if="!tipShow">{{tipTextEnd}}：</span>
      <!-- 天 -->
      <span v-if="msTime.day>0"><span style="color:red;">{{msTime.day}}</span><i>{{dayTxt}}</i></span>
      <!-- 时 -->
      <span style="color:red;">{{msTime.hour}}</span><i>{{hourTxt}}</i>
      <!-- 分 -->
      <span style="color:red;">{{msTime.minutes}}</span><i>{{minutesTxt}}</i>
      <!-- 秒 -->
      <span style="color:red;">{{msTime.seconds}}</span><i>{{secondsTxt}}</i>
    </p>
    <!-- 活动结束的提示 -->
    <p v-if="!msTime.show" class="grey_3">{{endText}}</p>
  </div>
</template>
<script>
export default {
