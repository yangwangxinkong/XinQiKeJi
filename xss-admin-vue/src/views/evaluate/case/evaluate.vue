<template>
  <div class="app-container">
    ...
    <el-table ref="multipleTable" :data="list" v-loading="listLoading" fit tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" @sort-change='sortChange'>
      ...
      <el-table-column prop="name" label="案例名称" width="200" sortable="custom" show-overflow-tooltip>
        <template slot-scope="scope">
          <a :title="scope.row.name" style="text-decoration:underline;" :href="'#/company/company_case/edit/'+scope.row.id">{{scope.row.name}}</a>
        </template>
      </el-table-column>

      <el-table-column prop="companyName" label="所属企业" width="120" sortable="custom" show-overflow-tooltip>
      </el-table-column>

      <el-table-column align="center" label="所属设计师" width="auto">
        <template slot-scope="scope">
          <span v-if="scope.row.designerName">{{scope.row.designerName}}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>

      <el-table-column label="人数">
        <el-table-column prop="commentNum" label="评论" width="80" show-overflow-tooltip>
        </el-table-column>

        <el-table-column prop="praiseNum" label="点赞" width="80" show-overflow-tooltip>
        </el-table-column>

        <el-table-column prop="scoreNum" label="评分" width="80" show-overflow-tooltip>
        </el-table-column>
      </el-table-column>
      
      <el-table-column prop="averageNum" label="评分平均数" width="100" show-overflow-tooltip>
      </el-table-column>

      <el-table-column align="center" label="查看" width="280">
        <template slot-scope="scope">
          <router-link :to="'/evaluate/case/comment/'+scope.row.id">
            <el-button type="primary" size="small" icon="el-icon-search">评论</el-button>
          </router-link>
          <router-link :to="'/evaluate/case/praise/'+scope.row.id">
            <el-button type="primary" size="small" icon="el-icon-search">点赞</el-button>
          </router-link>
          <router-link :to="'/evaluate/case/score/'+scope.row.id">
...
<script>
  ...
  import { fetchList } from '@/api/companyCase'

  export default {
    name: 'companyCaseTable',
    directives: {
      waves
    },
    data() {
