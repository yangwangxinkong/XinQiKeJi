<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <router-link :to="'/settings/pay_base/add'">
    ...
    <el-table ref="multipleTable" :data="list" v-loading="listLoading" fit tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" @sort-change='sortChange'>
      ...
      <el-table-column
        prop="providentBaseMin"
        label="公积金基数最小值"
        win-width="100"
        show-overflow-tooltip>
        <template slot-scope="scope">
          <span>￥{{scope.row.providentBaseMin}}</span>
        </template>
      </el-table-column>

      <el-table-column
        prop="providentBaseMax"
        label="公积金基数最大值"
        win-width="100"
        show-overflow-tooltip>
        <template slot-scope="scope">
          <span>￥{{scope.row.providentBaseMax}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="操作" width="120">
        <template slot-scope="scope">
          <router-link :to="'/settings/pay_base/edit/'+scope.row.id">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.listLoading = true
        get(url.payBaseList, this.listQuery).then(response => {
      ...
      handleDelte() {
        ...
        this.$confirm('删除无法恢复, 是否继续?', '提示', {
          ...
          post(url.payBaseDelete, {ids:ids}).then(response => {
