<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input @keyup.enter.native="handleFilter" style="width: 200px;" class="filter-item" placeholder="名称" v-model="listQuery.searchValue">
      </el-input>
      <el-button class="filter-item" type="primary" v-waves icon="el-icon-search" @click="handleFilter">{{$t('table.search')}}</el-button>
      <router-link :to="'/sys/shipping_method/add'">
    ...
    <el-table ref="multipleTable" :data="list" v-loading="listLoading" fit tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" @sort-change='sortChange'>
      ...
      <el-table-column align="center" label="操作" width="120">
        <template slot-scope="scope">
          <router-link :to="'/sys/shipping_method/edit/'+scope.row.id">
...
<script>
  ...
  import { getShippingMethodList,deleteShippingMethods } from '@/api/sys'

  export default {
    name: 'shippingMethodTable',
    directives: {
      waves
    },
    data() {
    ...
    methods: {
      getList() {
        ...
        getShippingMethodList(this.listQuery).then(response => {
      ...
      handleDelte() {
        ...
        this.$confirm('删除无法恢复, 是否继续?', '提示', {
          ...
          deleteShippingMethods({ids:ids}).then(response => {
