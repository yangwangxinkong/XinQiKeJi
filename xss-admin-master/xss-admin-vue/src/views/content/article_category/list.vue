<template>
  ...
  <router-link :to="'/content/article_category/add'">
    ...
    <tree-table :data="data" >
      ...
      <el-table-column align="center" label="名称" width="auto">
        <template slot-scope="scope">
          <span :style="'padding-left:' + ((scope.row.grade)*30+30) + 'px'">{{scope.row.name}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="编码" width="auto">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('table.actions')" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <router-link :to="'/content/article_category/edit/'+scope.row.id">
...
//            title: [{ required: true, message: 'title is required', trigger: 'blur' }]
    ...
    methods: {
      ...
      getList(){
          ...
          get(url.articleCategoryList,{}).then(response => {
      ...
      handleUpdate(row){
        get(url.articleCategoryInfo, {id:row.id}).then(response => {
      ...
      handleDelete(val){
        ...
        this.$confirm('删除无法恢复, 是否继续?', '提示', {
          ...
          get(url.articleCategoryDelete, {ids:this.id}).then(response => {
