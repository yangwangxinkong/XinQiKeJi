<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <router-link :to="'/content/article/add'">
    ...
    <el-table ref="multipleTable" :data="data" v-loading="listLoading" style="width: 100%" @selection-change="handleSelectionChange" >
      ...
      <el-table-column align="center" label="文章分类" width="auto">
        <template slot-scope="scope">
          <span>{{scope.row.articleCategory.name}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="是否发布" width="auto">
        <template slot-scope="scope">
          <span v-if="scope.row.isPublication">是</span>
          <span v-else>否</span>
        </template>
      </el-table-column>
