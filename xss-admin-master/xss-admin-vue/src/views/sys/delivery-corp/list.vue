    ...
    <el-table ref="multipleTable" :data="list" v-loading="listLoading" fit tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" @sort-change='sortChange'>
      ...
      <el-table-column align="center" label="操作" width="120">
        <template slot-scope="scope">
          <router-link :to="'/sys/delivery_corp/edit/'+scope.row.id">
...
<script>
  ...
  import { getDeliveryCorpList,deleteDeliveryCorps } from '@/api/sys'

  export default {
    name: 'deliveryCorpTable',
    directives: {
      waves
    },
    data() {
    ...
    methods: {
      getList() {
        ...
        getDeliveryCorpList(this.listQuery).then(response => {
      ...
      handleDelte() {
        ...
        this.$confirm('删除无法恢复, 是否继续?', '提示', {
          ...
          deleteDeliveryCorps({ids:ids}).then(response => {
