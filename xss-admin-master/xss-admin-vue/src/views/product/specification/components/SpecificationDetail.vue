    ...
    <el-form class="form-container" :model="postForm" :rules="rules" ref="postForm" label-width="100px" label-position="right">
      ...
      <div class="createPost-main-container">
        ...
        <el-form-item label="备注：" prop="memo">
          <el-col :span="5">
          <el-input placeholder="请输入备注" v-model="postForm.memo"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="排序：" prop="order">
          <el-col :span="5">
            <el-input-number style="width: 200px;" class="filter-item" placeholder="排序" v-model="postForm.order" name="order" :min="1">
            </el-input-number>
          </el-col>
        </el-form-item>
        <div class="filter-container">
          <el-button class="filter-item" @click="handleCreateSpecValue" type="success">添加规格值</el-button>
        </div>
        <el-table class="el-tb-edit" :data="specValueList" ref="specValueTable" border fit highlight-current-row empty-text="点击‘添加规格值’按钮" >
          <el-table-column prop="name" label="规格值名称">
            <template slot-scope="scope">
              <el-input v-model="scope.row.name" size="mini" controls-position="right"></el-input>
            </template>
          </el-table-column>
          <el-table-column prop="order" label="规格值排序">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="danger" @click="delSpecValue(scope.$index, scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
import { info,save } from '@/api/specification'
...
export default {
  ...
  methods: {
    ...
    handleCreateSpecValue() {
      var specValueObj = {
        name: "",
        order: ""
      };
      this.specValueList.push(specValueObj);
      setTimeout(() => {
        this.$refs.specValueTable.setCurrentRow(specValueObj);
      }, 10); //<==用于延时渲染后选中这行
    },
    delSpecValue(index, r) {
      console.log(index, r);
    ...
    fetchData(id) {
      info(id).then(response => {
        ...
        this.postForm.memo = data.memo
        this.postForm.order = data.order
        this.specValueList = data.specificationValues
    ...
    submitForm() {
      ...
      this.postForm.specificationValues = this.specValueList
      ...
      this.$refs.postForm.validate(valid => {
        if (valid) {
          ...
          var validFlag = true;
          for(var p in this.specValueList){
            if(this.specValueList[p].name == ""){
              validFlag = false;
            }
          }
          if (!validFlag){
          ...
          save(this.postForm).then(response => {
            if (response.data.result==='00000000') {
              ...
              this.$router.push({
              path:"/product/specification/list"})
            }else{
              this.$notify({
                title: '失败',
                message: '操作失败',
