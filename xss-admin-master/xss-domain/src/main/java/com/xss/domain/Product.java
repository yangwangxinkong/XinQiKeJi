...
import com.xss.domain.enums.ProductDelivery;
...
public class Product extends BaseEntity {
	...
	public static final String[] DEFAULT_JSON_PARAMS = new String[]{"id", "sn", "name", "title", "fullName", "image", "weight", "unit", "stock", "point", "exchangePoint", "isStoreFreight", "productDelivery", "commentNum","praiseNum"};
	...
	/** 是否卖家承担运费 */
	private Boolean isStoreFreight;
	...
	/** 商品运费 */
	private List<ProductFreight> productFreights = new ArrayList<ProductFreight>();
	/** 商品配送地区 */
	private List<ProductArea> productAreas = new ArrayList<ProductArea>();
	/** 商品配送站点 */
	private List<ProductStation> productStations = new ArrayList<ProductStation>();
	...
	/** 批发价 */
	private Map<Integer, BigDecimal> wholesalePrice = new HashMap<Integer, BigDecimal>();
...
//	@Field(store = Store.YES, index = Index.UN_TOKENIZED)
	@NotNull
	@Column(nullable = false)
	public Boolean getIsStoreFreight() {
		return isStoreFreight;
	}

	public void setIsStoreFreight(Boolean isStoreFreight) {
		this.isStoreFreight = isStoreFreight;
	}
...
//	@Field(store = Store.YES, index = Index.NO)
	...
	@Valid
	@ElementCollection
	@CollectionTable(name = "xx_product_product_freight")
	public List<ProductFreight> getProductFreights() {
		return productFreights;
	}

	public void setProductFreights(List<ProductFreight> productFreights) {
		this.productFreights = productFreights;
	}


	@Valid
	@ElementCollection
	@CollectionTable(name = "xx_product_product_area")
	public List<ProductArea> getProductAreas() {
		return productAreas;
	}

	public void setProductAreas(List<ProductArea> productAreas) {
		this.productAreas = productAreas;
	}

	@Valid
	@ElementCollection
	@CollectionTable(name = "xx_product_product_station")
	...
	@CollectionTable(name = "xx_product_wholesale_price")
	public Map<Integer, BigDecimal> getWholesalePrice() {
	 ...
	 *            批量价
	 */
	public void setWholesalePrice(Map<Integer, BigDecimal> wholesalePrice) {
...
//		}
	...
	public JSONArray convertList(List list, String[] params){
		...
		if (null != list && !list.isEmpty()){
			for (Object domain : list){
				JSONObject jo = convertEntity(domain, params);
				array.add(jo);
			}
		}
		return array;
	...
	@Override
	public JSONObject convertEntity(Object domain, String[] params){
		Product product = (Product)domain;
		JSONObject jo = super.convertEntity(product,DEFAULT_JSON_PARAMS);
		jo.put("seoTitle", product.getSeoTitle());
		jo.put("seoKeywords", product.getSeoKeywords());
		jo.put("seoDescription", product.getSeoDescription());
		jo.put("keyword", product.getKeyword());
		jo.put("memo", product.getMemo());
		jo.put("price", BigDecimalUtils.setScaleDown(product.getPrice(),2));
		jo.put("marketPrice", BigDecimalUtils.setScaleDown(product.getMarketPrice(),2));
		JSONObject productCategoryJo = JsonUtil.toJSONObject(product.getProductCategory(), new String[]{"id", "name"});
		jo.put("productCategory", productCategoryJo);
		JSONObject productTypeJo = new JSONObject();
		productTypeJo.put("name", product.getProductType().name());
		productTypeJo.put("desc", product.getProductType().getDesc());
		jo.put("productType", productTypeJo);
		jo.put("createDate", DateUtil.format((product.getCreateDate())));


		if (ArrayUtils.contains(params, "brand")){
			if(product.getBrand() != null){
				jo.put("brand",JsonUtil.toJSONObject(product.getBrand(), new String[]{"id", "name"}));
			}
		}
		if (ArrayUtils.contains(params, "thumbnail")){
			jo.put("thumbnail", org.apache.commons.lang.StringUtils.isBlank(product.getThumbnail()) ? "" : product.getThumbnail());
		}
		if (ArrayUtils.contains(params, "isMarketable")){
			jo.put("isMarketable", product.getIsMarketable());
		}
		if (ArrayUtils.contains(params, "isTop")){
			jo.put("isTop", product.getIsTop());
		}
		if (ArrayUtils.contains(params, "isGift")){
			jo.put("isGift", product.getIsGift());
		}
		if (ArrayUtils.contains(params, "isList")){
			jo.put("isList", product.getIsList());
		}
		if (ArrayUtils.contains(params, "isLimit")){
			jo.put("isLimit", product.getIsLimit());
		}
		if (ArrayUtils.contains(params, "deliveryTime")){
			jo.put("deliveryTime", product.getDeliveryTime());
		}
		if (ArrayUtils.contains(params, "deliveryPlace")){
			jo.put("deliveryPlace", product.getDeliveryPlace());
		}
		//有效库存
		if (ArrayUtils.contains(params, "availableStock")){
			jo.put("availableStock", product.getAvailableStock());
		}

		if (ArrayUtils.contains(params, "productImages")){
			JSONArray productImageJa = new JSONArray();
			if(product.getProductImages() != null && !product.getProductImages().isEmpty()){
				productImageJa = JsonUtil.toJSONArray(product.getProductImages(), new String[]{"id", "title", "source", "order"});
			}
			jo.put("productImages", productImageJa);
		}

		if (ArrayUtils.contains(params, "productFreights")){
			JSONArray productFreightJa = new JSONArray();
			if(product.getProductFreights() != null && !product.getProductFreights().isEmpty()){
				productFreightJa = JsonUtil.toJSONArray(product.getProductFreights(), new String[]{"minWeight", "maxWeight", "price"});
			}
			jo.put("productFreights", productFreightJa);
		}

        if (ArrayUtils.contains(params, "introduction")){
            jo.put("introduction", product.getIntroduction());
        }
		if(ArrayUtils.contains(params,"tagIds")){
		...
		//系列商品的规格,以商品为单位
		if (ArrayUtils.contains(params, "productSpecificationValues")){
			JSONArray specificationValueJa = new JSONArray();

			for(Product productTemp : product.getGoods().getProducts()) {
				JSONObject specificationValueJo = new JSONObject();
				specificationValueJo.put("productId", productTemp.getId());
				specificationValueJo.put("specificationValues", JsonUtil.toJSONArray(productTemp.getSpecificationValues(), new String[]{"id", "name"}));
				specificationValueJa.add(specificationValueJo);
			}
			jo.put("productSpecificationValues", specificationValueJa);
		}

		//当前商品的规格
		if (ArrayUtils.contains(params, "specificationValueIds")){
			jo.put("specificationIds", JsonUtil.toJSONArray(product.getSpecificationValues(), new String[]{"id", "name"}));
		}

		//当前商品的所选规格名
		if (ArrayUtils.contains(params, "specificationValueNames")){
			...
			for(Specification specification : product.getSpecifications()){
				...
				specificationJo.put("specificationId", specification.getId());
				specificationJo.put("specificationName", specification.getName());

				JSONArray specificationValueJa = new JSONArray();

				for(SpecificationValue specificationValue : specification.getSpecificationValues()) {
					if(product.getSpecificationValues() != null && product.getSpecificationValues().contains(specificationValue)) {
						specificationValueJa.add(JsonUtil.toJSONObject(specificationValue, new String[]{"id", "name"}));
					}
				}
				specificationJo.put("specificationValues", specificationValueJa);
				specificationJa.add(specificationJo);
			}
			jo.put("specificationValueNames", specificationJa);
		}

		//系列商品的所有规格 供选择用
		if (ArrayUtils.contains(params, "specificationValues_all")){
			...
			for(Specification specification : product.getSpecifications()){
				JSONObject specificationJo = new JSONObject();
				specificationJo.put("id", specification.getId());
				specificationJo.put("name", specification.getName());

				JSONArray specificationValueJa = new JSONArray();

				for(SpecificationValue specificationValue : specification.getSpecificationValues()) {
					if(specificationValues != null && specificationValues.contains(specificationValue)) {
//						specificationValueJa.add(JsonUtil.toJSONObject(specificationValue, new String[]{"id", "name"}));
						/**  小程序新增 */
						JSONObject specificationValueJo = JsonUtil.toJSONObject(specificationValue, new String[]{"id", "name"});
						specificationValueJo.put("checkd", false);
						for (SpecificationValue item:product.getSpecificationValues()) {
							if(specificationValue.getId().equals(item.getId())){
								specificationValueJo.put("checkd", true);
								break;
							}
						}
						specificationValueJa.add(specificationValueJo);
			...
			jo.put("specifications", specificationJa);
		}
		return jo;
	}
