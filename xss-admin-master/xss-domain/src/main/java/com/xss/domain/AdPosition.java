...
public class AdPosition extends BaseEntity {
	...
	public static final String[] DEFAULT_JSON_PARAMS = new String[]{"id", "name","width","height","description","template", "type"};
	...
	public AdPositionType getType() {
		return type;
	}

	public void setType(AdPositionType type) {
		this.type = type;
	}
	...
	public JSONObject convertEntity(Object entity, String[] params){
		AdPosition adPosition =((AdPosition) entity);
		JSONObject jo = super.convertEntity(entity,DEFAULT_JSON_PARAMS);
		jo.put("typeDesc", adPosition.getType().getDesc());
		if (ArrayUtils.contains(params,"value")) {
			jo.put("value",adPosition.getName()+"["+adPosition.getWidth()+"×"+adPosition.getHeight()+"]");
		}
		return jo;
	}
